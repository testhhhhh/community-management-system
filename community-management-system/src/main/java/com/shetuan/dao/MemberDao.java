...
public class MemberDao extends BaseDao {
	...
	public Member getMemberByName(String loginname) {
		...
		try {
			...
			getCon();
			//要执行的SQL
			String sql = "select l.login_name,m.member_name,m.member_gender,l.manager_id,m.member_grade,m.member_institute,m.member_add,m.member_phone,m.member_email,c.community_name,join_communityId,activity_point from member as m,login as l,community as c where m.login_name=l.login_name and  m.join_communityId=c.community_id and l.login_name=?";
			//执行查询，并传入参数：登录名
			...
			while (rs.next()) {
				...
				member.setMemberEmail(rs.getString("member_email"));
				member.setJoinCommunity(rs.getString("community_name"));
				member.setJoint_communityId(rs.getInt("join_communityId"));
				member.setActivity_point(rs.getInt("activity_point"));
		...
		try {
			...
			if (l == null) {//判断该登录名是否可用
				try {
					...
					String sql1 = "insert into member(login_name,member_email,member_phone,join_communityId,manager_id)values(?,?,?,0,0)";
					//获取SQL执行结果
	...
	public int updateMember(Member member) {
		...
		try {
			...
			getCon();
			//要执行的更新SQL
			String sql = "update member set member_name=?,member_gender=?,member_grade=?,member_institute=?,member_add=?,member_email=?,member_phone=?,creat_community=?,join_communityId=? where login_name=?";
			//执行SQL传入参数，并获取手影响行数
	...
	public List<Member> getMemberForSerach(String membername, Page page) {
		...
		try {
			...
			String countsql = "select count(1) " +
					"from member as m,login as l,community as c " +
					"where m.login_name=l.login_name and  m.join_communityId=c.community_id and m.member_name " +
					"like ? ";
			//执行SQL 并传入参数
	...
	public int addpoint(String login_name) {
		int result = 0;
		try {
			//获取连接
			getCon();
			//要执行的更新SQL
			String sql = "update member set activity_point=activity_point+1 where login_name=?";
			//执行SQL传入参数，并获取手影响行数
			result = exeUpdate(sql,
					new Object[] { login_name });
		} catch (SQLException e) {
			// TODO Auto-generated catch blockUpda
			e.printStackTrace();
		} finally {
			//关闭所有连接
			closeAll();
